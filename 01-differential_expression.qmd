...
For the protein-protein interaction networks we want the differential gene list for our various contrats
...
Note that repetative code is generally a bad sign suggesting you should stop and consider making a more generalisable function that you can reuse.
...
```{r}
# Add your code to functionalise the extraction of results dataframes from dds here
...
# Now, pairwise_combinations is a list of vectors, each containing a pair of conditions

# Generate a list to hold the contrast results
results_list <- setNames(vector("list", length(pairwise_combinations)), 
                         sapply(pairwise_combinations, function(x) paste(x, collapse = "_vs_")))
