...
Some peaks seem implausibly far from genes, let's restrict to +/- 1000 nt away - how many peaks do we lose?

```{r}
filt_atac_annot = atac_annot %>% filter(abs(Distance.to.TSS) <= 1000)
```

Are any of these ATAC peaks associated with our unique brain genes?

```{r}
filt_atac_annot = atac_annot %>% mutate(brain_enriched=Entrez.ID %in% brain_enriched)
filt_atac_annot %>%
...
Let's resize our peaks to be 100bp long (+/- 50 from peak centers) and get the sequences.

```{r}
brain_peaks_resized = resize(brain_peaks, width = 100, fix='center')
...
```{r}
memes::get_sequence(brain_peaks_resized, mouse.genome)
```
...
# Convert Ensembl IDs to Entrez IDs for all the genes in the list
results_list_entrez <- lapply(results_list, function(df) {
  genes <- df$gene
  entrez_ids <-
    getLDS(
  ...
  df <-
    dplyr::left_join(df, unique(entrez_ids), by = join_by(gene == ensembl_gene_id))
})
```
...
MAGMA wants an input file with each row denoting a group to test (in single cell seq data this is typically celltypes). The row starts with the group label followed by a space and then a space seperated list of the entrez IDs, so now we get our data into that format and save each percent to a different file.

```{r}
# Merge list to one df and take the relevant columns
magma_genes <- purrr::list_rbind(top_genes_list) |>
  dplyr::select(entrez_id, contrast, percent_genes)
...
lapply(names(dfs_by_percent), function(percent) {
  ...
  groups <- split(subset_df$entrez_id, subset_df$contrast)
